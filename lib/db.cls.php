...
class db2
{
	...
	public function connect($host = DH,$dbuser = DU,$password = DP,$dbname = DB,$dbcode = HE)
    {
            ...
            if(!$this->linkid)
            {
            	$this->linkid = mysqli_connect($host, $dbuser, $password) or die('Mysql数据库连接失败，请检查数据库用户名和密码是否正确！');
            }
            $version = $this->getVersion();

            if($version >= '4.0.1')
            {
            	mysqli_query("SET character_set_connection='{$dbcode}', character_set_results='{$dbcode}', character_set_client=binary", $this->linkid);
				if($version >= '5.0.1')
				{
					mysqli_query("SET sql_mode=''", $this->linkid);
				}
            }
    ...
    public function _fetch($sql = false,$type = 1,$position = 0)
    {
    	...
    	if(!mysqli_num_rows($this->queryid))return false;
