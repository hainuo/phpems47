...
class html
{
    ...
    public function buildHtml($fields, $values = null)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($field['fieldhtmltype'] != 'auto') {
                ...
                if (is_array($values)) $field['fieldhtmlproperty'][] = ['key' => 'value', 'value' => $values[$field['field']]];
                elseif ($field['fielddefault']) $field['fieldhtmlproperty'][] = ['key' => 'value', 'value' => $field['fielddefault']];
            ...
            if (is_array($values)) {
                $tmp['html'] = $this->{$field['fieldhtmltype']}(['pars' => $field['fieldhtmlproperty'], 'values' => $field['fieldvalues'], 'default' => $values[$field['field']]]);
            } else {
                $tmp['html'] = $this->{$field['fieldhtmltype']}(['pars' => $field['fieldhtmlproperty'], 'values' => $field['fieldvalues'], 'default' => $field['fielddefault']]);
            }
            $forms[] = $tmp;
        }
        return $forms;
    ...
    {
        if (!$args['default']) $args['pars'][] = ['key' => 'value', 'value' => date("Y-m-d H:i:s")];
        else
    ...
    {
        $pram = 0;
        if (!$args['default']) $args['pars'][] = ['key' => 'value', 'value' => date("Y-m-d")];
        else
            foreach ($args['pars'] as $id => $p) {
                if ($p['key'] == 'value') {
        ...
        if (!$pram) $args['pars'][] = ['key' => 'class', 'value' => 'datepicker'];
        return $this->text($args);
    }
