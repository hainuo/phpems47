    ...
    public function user()
    {
        ...
        switch ($subaction) {
            ...
            default:
                ...
                $args = [];
                if ($search['userid']) $args[] = ['AND', "userid = :userid", 'userid', $search['userid']];
                if ($search['username']) $args[] = ['AND', "username LIKE :username", 'username', '%' . $search['username'] . '%'];
                if ($search['useremail']) $args[] = ['AND', "useremail  LIKE :useremail", 'useremail', '%' . $search['useremail'] . '%'];
                if ($search['groupid']) $args[] = ['AND', "usergroupid = :usergroupid", 'usergroupid', $search['groupid']];
    ...
    public function ajax()
    {
        ...
        switch ($subaction) {
            ...
            case 'creatRandomUsers'://随机生成用户
                ...
                //2. 用户名密码生成规则
                //「createUUID」+[用户次序]+random(10-99)
                //  todo make the uuid is real uuid,beacause useless  i tried once.
                $createUUID = uniqid('U');
                $repeatResult = $this->user->getNumsByUUID($createUUID);
                if ($repeatResult > 0) {
                    $createUUID = uniqid('G');
                }
                for ($i = 0; $i < 100; $i++) {
                ...
                //写入文件开始
                $path = 'files/attach/users/' . date('ymd') . '/' . $createUUID . '.csv';
